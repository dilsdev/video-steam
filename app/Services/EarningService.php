...
class EarningService
{
    ...
    public function calculateDailyEarnings(?string $date = null): array
    {
        ...
        foreach ($viewsByVideo as $item) {
            ...
            // Hitung penghasilan kotor: (views / 1000) * CPM
            $grossAmount = ($item->views_count / 1000) * $cpmRate;

            // Platform mengambil fee (default 50%), kreator dapat sisanya
            $platformFeePercent = Setting::get('platform_fee_percent', 50);
            $amount = $grossAmount * ((100 - $platformFeePercent) / 100);
