        ...
        @if (!$skipAds)
            ...
            <script type="text/javascript">
                ...
                runAdcashWhenReady(function() {
                    console.log('Adcash Lib Ready, Executing Zones...');

                    // 1. Jalankan AutoTag (High Priority untuk Pop/Interstitial)
                    // Pastikan env ADCASH_MODE = 'both' atau 'auto'
                    @if (in_array(config('adcash.mode'), ['auto', 'both']))
                        aclib.runAutoTag({
                            zoneId: '{{ config('adcash.autotag_zone_id') }}'
                        });
                    @endif

                    // 2. Jalankan Manual Banner & Sidebar (Jika mode allow manual)
                    @if (in_array(config('adcash.mode'), ['manual', 'both']))

                        // Banner Top (Pastikan element ID ada)
                        if (document.getElementById('ad-banner-top')) {
                            aclib.runBanner({
                                zoneId: '{{ config('adcash.banner_zone_id') }}',
                                container: document.getElementById('ad-banner-top') // Target spesifik ke ID
                            });
                        }

                        // Sidebar (Pastikan element ID ada)
                        if (document.getElementById('ad-sidebar-right')) {
                            aclib.runBanner({
                                zoneId: '{{ config('adcash.sidebar_zone_id') }}',
                                container: document.getElementById('ad-sidebar-right') // Target spesifik ke ID
                            });
                        }
